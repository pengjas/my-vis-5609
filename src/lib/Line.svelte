<script lang="ts">
	...
	const yScale = $derived(
		d3
			.scaleLinear()
			.domain([0, d3.max(data, (d) => d.y) as number]) // Domain starts at 0 for a better view
	...
	function handleBrush(event: any) {
		...
		if (selection) {
			...
			yearRange = undefined; // Reset the range if the brush is cleared
		}
	}
	...
	function setupBrush() {
		const brush = d3
			...
			.on("end", handleBrush); // Use "end" event to update only when brushing is complete
...
<svg {width} {height} class="line">
	...
	<g class="points">
		{#each data as point (point.x)}
			<circle cx={xScale(point.x)} cy={yScale(point.y)} r="2" fill="black" />
		{/each}
	</g>
